...
class CartControllerCore extends FrontController {
    ...
    protected function processChangeProductInCart() {
        ...
        if ($this->id_product_attribute) {
            ...
            if (!Product::isAvailableWhenOutOfStock($product->out_of_stock) && !Attributes::checkAttributeQty($this->id_product_attribute, $qtyToCheck)) {
            ...
            } else if (!Product::isAvailableWhenOutOfStock($product->out_of_stock) && !Attributes::checkAttributeQty($this->id_product_attribute, $qtyToCheck)) {
                $this->errors[] = Tools::displayError('There aren\'t enough products in stock.', !Tools::getValue('ajax'));
            }

        } else if (!$product->checkQty($qtyToCheck)) {
            $this->errors[] = Tools::displayError('There aren\'t enough products in stock.', !Tools::getValue('ajax'));
        }
        ...
        if (!$this->errors && $mode == 'add') {
            ...
            if (!$this->errors) {
                ...
                if ($updateQuantity < 0) {
                    ...
                    $minimalQuantity = ($this->id_product_attribute) ? Attributes::getAttributeMinimalQty($this->id_product_attribute) : $product->minimal_quantity;
                    ...
                    if (count($cartRules2) != count($cartRules)) {
                        ...
                        if (count($availableCartRules2) != count($availableCartRules)) {
                            $this->ajax_refresh = true;
                        } else if (count($availableCartRules2)) {
                            $ruleList = [];
    ...
    protected function processDeleteProductInCart() {
        ...
        if (count($customizationProduct)) {
            ...
            if ($this->id_product_attribute > 0) {
                $minimalQuantity = (int) Attributes::getAttributeMinimalQty($this->id_product_attribute);
            } else {
