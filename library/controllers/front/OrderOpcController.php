...
class OrderOpcControllerCore extends ParentOrderController {
                ...
                if (Tools::isSubmit('method')) {
                    ...
                    case 'updateAddressesSelected':
                        ...
                        if ($this->context->customer->isLogged(true)) {
                            ...
                            } else if (!Address::isCountryActiveById((int) Tools::getValue('id_address_delivery'))) {
                                $this->errors[] = Tools::displayError('This address is not in a valid area.');
                            } else if (!Validate::isLoadedObject($addressDelivery) || !Validate::isLoadedObject($addressInvoice) || $addressInvoice->deleted || $addressDelivery->deleted) {
                    ...
                    default:
                        throw new PhenyxShopException('Unknown method "' . Tools::getValue('method') . '"');
                    }

                } else {
                    throw new PhenyxShopException('Method is not defined');
                }
    ...
    protected function _getCarrierList() {
        ...
        if (!Address::isCountryActiveById((int) $this->context->cart->id_address_delivery) && $this->context->cart->id_address_delivery != 0) {
            $this->errors[] = Tools::displayError('This address is not in a valid area.');
        } else if ((!Validate::isLoadedObject($addressDelivery) || $addressDelivery->deleted) && $this->context->cart->id_address_delivery != 0) {
            $this->errors[] = Tools::displayError('This address is invalid.');
        } else {
            $result = [
                'HOOK_BEFORECARRIER' => Hook::exec(
