...
class MySQLCore extends Db {
    ...
    public function connect() {
        ...
        if (!$this->link = mysql_connect($this->server, $this->user, $this->password)) {
            throw new PhenyxShopDatabaseException(Tools::displayError('Link to database cannot be established.'));
        }

        if (!$this->set_db($this->database)) {
            throw new PhenyxShopDatabaseException(Tools::displayError('The database selection cannot be made.'));
        }

        // UTF-8 support

        if (!mysql_query('SET NAMES \'utf8\'', $this->link)) {
            throw new PhenyxShopDatabaseException(Tools::displayError('PhenyxShop Fatal error: no utf-8 support. Please check your server configuration.'));
        }
