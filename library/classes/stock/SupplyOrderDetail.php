...
class SupplyOrderDetailCore extends ObjectModel
{
    ...
    protected function calculatePrices()
    {
        ...
        $this->price_te = Tools::EPH_round((float) $this->unit_price_te * (int) $this->quantity_expected, 6);
        ...
        if ($this->discount_rate != null && (is_float($this->discount_rate) || is_numeric($this->discount_rate)) && $this->discount_rate > 0) {
            $this->discount_value_te = Tools::EPH_round((float) $this->price_te * ($this->discount_rate / 100), 6);
        }

        // calculates entry price with discount
        $this->price_with_discount_te = Tools::EPH_round($this->price_te - $this->discount_value_te, 6);

        // calculates tax value
        $this->tax_value = Tools::EPH_round($this->price_with_discount_te * ((float) $this->tax_rate / 100), 6);
        $this->price_ti = Tools::EPH_round($this->price_with_discount_te + $this->tax_value, 6);

        // defines default values for order discount fields
        $this->tax_value_with_order_discount = Tools::EPH_round($this->tax_value, 6);
        $this->price_with_order_discount_te = Tools::EPH_round($this->price_with_discount_te, 6);
    }
    ...
    public function applyGlobalDiscount($discountRate)
    {
        if ($discountRate != null && is_numeric($discountRate) && (float) $discountRate > 0) {
            ...
            $this->price_with_order_discount_te = Tools::EPH_round($discount_value, 6);

            // calculates new tax value, with global order discount
            $this->tax_value_with_order_discount = Tools::EPH_round($this->price_with_order_discount_te * ((float) $this->tax_rate / 100), 6);
