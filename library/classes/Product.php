...
class ProductCore extends PhenyxObjectModel {
    ...
    public static $definition = [
        ...
        'fields'         => [
            ...
            'id_tax_rules_group'        => ['type' => self::TYPE_INT,  'validate' => 'isUnsignedId', 'dbNullable' => false],
            'on_sale'                   => ['type' => self::TYPE_BOOL,  'validate' => 'isBool', 'dbDefault' => '0'],
            'online_only'               => ['type' => self::TYPE_BOOL,  'validate' => 'isBool', 'dbDefault' => '0'],
            'ean13'                     => ['type' => self::TYPE_STRING, 'validate' => 'isEan13', 'size' => 13],
            'upc'                       => ['type' => self::TYPE_STRING, 'validate' => 'isUpc', 'size' => 12],
            'ecotax'                    => ['type' => self::TYPE_FLOAT,  'validate' => 'isPrice', 'dbDefault' => '0.000000'],
            'quantity'                  => ['type' => self::TYPE_INT, 'validate' => 'isInt', 'dbDefault' => '0', 'dbType' => 'int(10)'],
            'minimal_quantity'          => ['type' => self::TYPE_INT,  'validate' => 'isUnsignedInt', 'dbDefault' => '1'],
            'price'                     => ['type' => self::TYPE_FLOAT,  'validate' => 'isPrice', 'required' => true, 'dbDefault' => '0.000000'],
            'wholesale_price'           => ['type' => self::TYPE_FLOAT,  'validate' => 'isPrice', 'dbDefault' => '0.000000'],
            'unity'                     => ['type' => self::TYPE_STRING,  'validate' => 'isString'],
            'unit_price_ratio'          => ['type' => self::TYPE_FLOAT,  'dbDefault' => '0.000000'],
            'additional_shipping_cost'  => ['type' => self::TYPE_FLOAT,  'validate' => 'isPrice', 'dbDefault' => '0.000000'],
            ...
            'customizable'              => ['type' => self::TYPE_INT,  'validate' => 'isUnsignedInt', 'dbType' => 'tinyint(2)', 'dbDefault' => '0'],
            'uploadable_files'          => ['type' => self::TYPE_INT,  'validate' => 'isUnsignedInt', 'dbType' => 'tinyint(4)', 'dbDefault' => '0'],
            'text_fields'               => ['type' => self::TYPE_INT,  'validate' => 'isUnsignedInt', 'dbType' => 'tinyint(4)', 'dbDefault' => '0'],
            'active'                    => ['type' => self::TYPE_BOOL,  'validate' => 'isBool', 'dbDefault' => '0'],
            'redirect_type'             => ['type' => self::TYPE_STRING,  'validate' => 'isString', 'values' => ['', '404', '301', '302'], 'dbDefault' => ''],
            'id_product_redirected'     => ['type' => self::TYPE_INT,  'validate' => 'isUnsignedId', 'dbDefault' => '0'],
            'available_for_order'       => ['type' => self::TYPE_BOOL,  'validate' => 'isBool', 'dbType' => 'tinyint(1)', 'dbDefault' => '1'],
            'available_date'            => ['type' => self::TYPE_DATE,  'validate' => 'isDateFormat', 'dbDefault' => '1970-01-01', 'dbType' => 'date'],
            'condition'                 => ['type' => self::TYPE_STRING,  'validate' => 'isGenericName', 'values' => ['new', 'used', 'refurbished'], 'default' => 'new'],
            'show_price'                => ['type' => self::TYPE_BOOL,  'validate' => 'isBool', 'dbType' => 'tinyint(1)', 'dbDefault' => '1'],
            'indexed'                   => ['type' => self::TYPE_BOOL,  'validate' => 'isBool', 'dbType' => 'tinyint(1)', 'dbDefault' => '0'],
            'visibility'                => ['type' => self::TYPE_STRING,  'validate' => 'isProductVisibility', 'values' => ['both', 'catalog', 'search', 'none'], 'default' => 'both'],
            ...
            'advanced_stock_management' => ['type' => self::TYPE_BOOL,  'validate' => 'isBool', 'dbType' => 'tinyint(1)', 'dbDefault' => '0'],
            'pack_stock_type'           => ['type' => self::TYPE_INT,  'validate' => 'isUnsignedInt', 'dbDefault' => '3'],
        ...
        'associations'   => [
            ...
            'categories'       => ['type' => self::HAS_MANY, 'object' => 'Category', 'joinTable' => 'category_product'],
            'stock_availables' => ['type' => self::HAS_MANY, 'field' => 'id_product', 'foreignField' => 'id_product', 'object' => 'StockAvailable'],
            'accessories'      => ['type' => self::HAS_MANY, 'object' => 'Product', 'joinTable' => 'accessory', 'joinSourceField' => 'id_product_1', 'joinTargetField' => 'id_product_2'],
        ],
     ...
     */
    public function __construct($idProduct = null, $full = false, $idLang = null, Context $context = null) {

        parent::__construct($idProduct, $idLang);
    ...
    public static function priceCalculation(
        ...
        $cacheId = (int) $idProduct .  '-' . (int) $idCurrency . '-' . (int) $idCountry . '-' . $idState . '-' . $zipcode . '-' . (int) $idGroup .
        ...
        if (!isset(static::$_pricesLevel2[$cacheId2])) {
            ...
            if (Combination::isFeatureActive()) {
                $sql->select('IFNULL(pa.id_product_attribute,0) id_product_attribute, pa.`price` AS attribute_price, pa.default_on');
     ...
     * @throws PrestaShopException
     */
    public static function sqlStock($productAlias, $productAttribute = null, $innerJoin = false, Company $shop = null) {

        $idCompany = ($shop !== null ? (int) $shop->id : null);
    ...
    public static function getNewProducts($idLang, $pageNumber = 0, $nbProducts = 10, $count = false, $orderBy = null, $orderWay = null, Context $context = null) {
        ...
        if ($count) {
            $sql = 'SELECT COUNT(p.`id_product`) AS nb
                    FROM `' . _DB_PREFIX_ . 'product` p
                    WHERE p.`active` = 1
                    AND p.`date_add` > "' . date('Y-m-d', strtotime('-' . (Configuration::get('EPH_NB_DAYS_NEW_PRODUCT') ? (int) Configuration::get('EPH_NB_DAYS_NEW_PRODUCT') : 20) . ' DAY')) . '"
                    ' . ($front ? ' AND p.`visibility` IN ("both", "catalog")' : '') . '
                    ' . $sqlGroups;
        ...
        $sql->select(
            ...
            pl.`meta_keywords`, pl.`meta_title`, pl.`name`, pl.`available_now`, pl.`available_later`, i.`id_image` id_image, il.`legend`, m.`name` AS manufacturer_name,
        ...
        $sql->where('p.`date_add` > "' . date('Y-m-d', strtotime('-' . (Configuration::get('EPH_NB_DAYS_NEW_PRODUCT') ? (int) Configuration::get('EPH_NB_DAYS_NEW_PRODUCT') : 20) . ' DAY')) . '"');
        ...
        if (Combination::isFeatureActive()) {
            $sql->select('p.minimal_quantity AS product_attribute_minimal_quantity, IFNULL(pa.id_product_attribute,0) id_product_attribute');
            $sql->leftJoin('product_attribute', 'pa', 'p.`id_product` = p.`id_product` AND pa.`default_on` = 1');
        }
    ...
    public static function getRandomSpecial($idLang, $beginning = false, $ending = false, Context $context = null) {
        ...
        if ($productReductions) {
            ...
            $sql = 'SELECT p.id_product, IFNULL(pa.id_product_attribute,0) id_product_attribute
                    FROM
                    `' . _DB_PREFIX_ . 'product_reductions` pr,
                    `' . _DB_PREFIX_ . 'product` p
                    LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute` pa
                        ON (p.`id_product` = pa.`id_product` AND pa.`default_on` = 1)
                    WHERE p.id_product= pr.id_product AND (pr.id_product_attribute = 0 OR pa.id_product_attribute = pr.id_product_attribute) AND p.`active` = 1
                        ' . $sqlGroups . '
                    ' . ($front ? ' AND p.`visibility` IN ("both", "catalog")' : '') . '
                    ORDER BY RAND()';
            ...
            $sql = 'SELECT p.*,  stock.`out_of_stock` out_of_stock, pl.`description`, pl.`description_short`,
                        ...
                        p.`ean13`, p.`upc`, i.`id_image` id_image, il.`legend`,
    ...
    public static function getPricesDrop(
        ...
        $sql = '
        SELECT
            p.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, pl.`description`, pl.`description_short`, pl.`available_now`, pl.`available_later`,
            IFNULL(pa.id_product_attribute, 0) id_product_attribute,
            pl.`link_rewrite`, pl.`meta_description`, pl.`meta_keywords`, pl.`meta_title`,
            pl.`name`, i.`id_image` id_image, il.`legend`, m.`name` AS manufacturer_name,
            DATEDIFF(
                p.`date_add`,
                DATE_SUB(
                    "' . date('Y-m-d') . ' 00:00:00",
        ...
        LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute` pa
            ON (p.`id_product` = pa.`id_product` AND pa.`default_on` = 1)
        ' . static::sqlStock('p', 0, false, $context->company) . '
        LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` pl ON (
            p.`id_product` = pl.`id_product`
            AND pl.`id_lang` = ' . (int) $idLang .  '
        )
    ...
    public static function isDiscounted($idProduct, $quantity = 1, Context $context = null) {
        ...
        return (bool) SpecificPrice::getSpecificPrice((int) $idProduct,  $idCurrency, $idCountry, $idGroup, $quantity, null, 0, 0, $quantity);
    }
    ...
    public static function getAttributesColorList(array $products, $haveStock = true) {
        ...
        if (!$res = Db::getInstance()->executeS(
            ...
            FROM `' . _DB_PREFIX_ . 'product_attribute` pa
            ' . ($checkStock ? static::sqlStock('pa', 'pa') : '') . '
    ...
    public static function searchByName($idLang, $query, Context $context = null) {
        ...
        $sql = new DbQuery();
        $sql->select('p.`id_product`, pl.`name`, p.`ean13`, p.`upc`, p.`active`, p.`reference`, m.`name` AS manufacturer_name, stock.`quantity`, p.advanced_stock_management, p.`customizable`');
        $sql->from('product', 'p');
        $sql->leftJoin(
            'product_lang',
            'pl',
    ...
    public static function duplicateAttributes($idProductOld, $idProductNew) {
        ...
        foreach ($result as $row) {
            ...
            $combination = new Combination($idCombination, null);
            ...
            // Set stock quantity
            StockAvailable::setQuantity((int) $idProductNew, $idProductAttributeNew, (int) $quantityAttributeOld);
     ...
     * @version 1.0.0 Initial version
     */
    protected static function _getCustomizationFieldsNLabels($productId, $idCompany = null) {
     ...
     * @version 1.0.0 Initial version
     */
    public static function getAllCustomizedDatas($idCart, $idLang = null, $onlyInCart = true, $idCompany = null) {
    ...
    public static function getProductName($idProduct, $idProductAttribute = null, $idLang = null) {
        ...
        if ($idProductAttribute) {
            ...
            $query->innerJoin('product_lang', 'pl', 'pl.id_product = pa.id_product AND pl.id_lang = ' . (int) $idLang );
     ...
     * @throws PrestaShopException
     */
    public static function getRealQuantity($idProduct, $idProductAttribute = 0, $idWarehouse = 0, $idCompany = null) {
        ...
        if (Configuration::get('EPH_ADVANCED_STOCK_MANAGEMENT') && static::usesAdvancedStockManagement($idProduct) &&
            StockAvailable::dependsOnStock($idProduct)
        ) {
    ...
    public static function setPackStockType($idProduct, $packStockType) {
        ...
        return Db::getInstance()->execute(
            'UPDATE ' . _DB_PREFIX_ . 'product p SET p.pack_stock_type = ' . (int) $packStockType . ' WHERE p.`id_product` = ' . (int) $idProduct
        );
    }
    ...
    public function updatePosition($way, $position) {
        ...
        $result = (Db::getInstance()->execute(
            ...
            p.`date_upd` = "' . date('Y-m-d H:i:s') . '", p.`date_upd` = "' . date('Y-m-d H:i:s') . '"
            WHERE cp.`position`
            ' . ($way
    ...
    public static function cleanPositions($idCategory, $position = 0) {
        ...
        $return &= Db::getInstance()->execute('
            ...
            SET p.`date_upd` = "' . $now . '", p.`date_upd` = "' . $now . '"
        ');
    ...
    public function addAttribute(
        ...
        $combination->price = round($price, 6);
        $combination->ecotax = round($ecotax, 6);
        ...
        if (count($idCompanyList)) {
            $combination->id_shop_list = array_unique($idCompanyList);
        }
    ...
    public function productAttributeExists($attributesList, $currentProductAttribute = false, Context $context = null, $allShops = false, $returnId = false) {
        ...
        $result = Db::getInstance()->executeS(
            ...
            FROM `' . _DB_PREFIX_ . 'product_attribute` pa
            JOIN `' . _DB_PREFIX_ . 'product_attribute` pas ON (pas.id_product_attribute = pa.id_product_attribute)
    ...
    public function update($nullValues = false) {
        ...
        if (Configuration::get('EPH_ADVANCED_STOCK_MANAGEMENT') && StockAvailable::dependsOnStock($this->id)) {
            Db::getInstance()->update(
                'stock',
                [
    ...
    public function addCombinationEntity(
        ...
        $idProductAttribute = $this->addAttribute($price, $weight, $unitImpact, $ecotax, $idImages, $reference, $ean13, $default, $location, $upc, $minimalQuantity, $idCompanyList, $availableDate);
    ...
    public function hasAttributesInOtherShops() {
        ...
        return (bool) Db::getInstance(_EPH_USE_SQL_SLAVE_)->getValue(
            ...
            SELECT pa.id_product_attribute
            FROM `' . _DB_PREFIX_ . 'product_attribute` pa
            LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute` pas ON (pa.`id_product_attribute` = pas.`id_product_attribute`)
    ...
    public function updateAttribute(
        ...
        if (count($idCompanyList)) {
            $combination->id_shop_list = $idCompanyList;
        }
    ...
    public function getAttributeCombinationsById($idProductAttribute, $idLang) {
        ...
        $sql = 'SELECT pa.*, ag.`id_attribute_group`, ag.`is_color_group`, agl.`name` AS group_name, al.`name` AS attribute_name, pal.`name`, pal.`description`, pal.`description_short`,
                    a.`id_attribute`
                FROM `' . _DB_PREFIX_ . 'product_attribute` pa
				LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_lang` pal ON pal.`id_product_attribute` = pa.`id_product_attribute` AND pal.`id_lang` = '. (int) $idLang . '
                LEFT JOIN `' . _DB_PREFIX_ . 'product_attribute_combination` pac ON pac.`id_product_attribute` = pa.`id_product_attribute`
                LEFT JOIN `' . _DB_PREFIX_ . 'attribute` a ON a.`id_attribute` = pac.`id_attribute`
                LEFT JOIN `' . _DB_PREFIX_ . 'attribute_group` ag ON ag.`id_attribute_group` = a.`id_attribute_group`
                LEFT JOIN `' . _DB_PREFIX_ . 'attribute_lang` al ON (a.`id_attribute` = al.`id_attribute` AND al.`id_lang` = ' . (int) $idLang . ')
    ...
    */
    ...
    public function checkDefaultAttributes() {
        ...
        if (!PhenyxObjectModel::updateMultishopTable('Combination', ['default_on' => 1], 'a.id_product_attribute = ' . (int) $mini['id_attr'])) {
            return false;
        }
    ...
    public function getAttributesGroups($idLang) {
        ...
        $sql = 'SELECT ag.`id_attribute_group`, ag.`is_color_group`, agl.`name` AS group_name, agl.`public_name` AS public_group_name,
                    ...
                    IFNULL(stock.quantity, 0) AS quantity, pa.`price`, pa.`ecotax`, pa.`weight`,
                    pa.`default_on`, pa.`reference`, pa.`unit_price_impact`,
                    pa.`minimal_quantity`, pa.`available_date`, ag.`group_type`, pal.name as attributeName, pal.description, pal.description_short
                FROM `' . _DB_PREFIX_ . 'product_attribute` pa
    ...
    public function getAccessories($idLang, $active = true) {
        ...
        $sql = 'SELECT p.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, pl.`description`, pl.`description_short`, pl.`link_rewrite`,
                    ...
                    i.`id_image` id_image, il.`legend`, m.`name` as manufacturer_name, cl.`name` AS category_default, IFNULL(pa.id_product_attribute, 0) id_product_attribute,
                    DATEDIFF(
                        p.`date_add`,
                        DATE_SUB(
                            "' . date('Y-m-d') . ' 00:00:00",
            ...
            ($active ? ' AND p.`active` = 1 AND p.`visibility` != \'none\'' : '') . '
    ...
    protected function _createLabel($languages, $type) {
        ...
        foreach ($languages as $language) {
            ...
            foreach ([Context::getContext()->company->id] as $idCompany) {
                $values .= '(' . (int) $idCustomizationField . ', ' . (int) $language['id_lang'] . ', ' . $idCompany . ',\'\'), ';
            }
    ...
    public function updateLabels() {
        ...
        if ($hasRequiredFields && !PhenyxObjectModel::updateMultishopTable('product', ['customizable' => 2], 'a.id_product = ' . (int) $this->id)) {
            return false;
        }
     ...
     * @version 1.0.0 Initial version
     */
    public function getCustomizationFields($idLang = false, $idCompany = null) {
    ...
    public static function getAttributesParams($idProduct, $idProductAttribute) {
        ...
        $idLang = (int) Context::getContext()->language->id;
        $cacheId = 'Product::getAttributesParams_' . (int) $idProduct . '-' . (int) $idProductAttribute . '-' . (int) $idLang;
    ...
    public function getParentCategories($idLang = null) {
        ...
        $sql = new DbQuery();
        $sql->from('category', 'c');
        $sql->leftJoin('category_lang', 'cl', 'c.id_category = cl.id_category AND id_lang = ' . (int) $idLang );
    ...
    */
     ...
     * @throws PrestaShopException
     */
    public function isColorUnavailable($idAttribute, $idCompany) {
        ...
        return Db::getInstance()->getValue(
            ...
            ' . StockAvailable::addSqlShopRestriction(null, $idCompany, 'sa') . '
            AND EXISTS (
                SELECT 1
                FROM ' . _DB_PREFIX_ . 'product_attribute pa
    ...
    public static function processTableSchema($table) {
        ...
        if ($table->getNameWithoutPrefix() === 'product_lang') {
            $table->reorderColumns(['id_product',  'id_lang']);
        }

        
    }
	
	public static function existInDatabase($idProduct) {
		
		
		$product = Db::getInstance()->getRow('SELECT * FROM `' . _DB_PREFIX_ . 'product` WHERE `id_product` = ' .(int)$idProduct);
