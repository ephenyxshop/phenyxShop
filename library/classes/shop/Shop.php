...
class ShopCore extends PhenyxObjectModel
{
    ...
    public function add($autoDate = true, $nullValues = false)
    {
        ...
        // @codingStandardsIgnoreStart
		Configuration::updateValue('EPH_ROUTE_product_rule', array_map(function() {return '{categories:/}{id}-{rewrite}';}, $langs));
        ...
        if (!($idShop = Tools::getValue('id_shop')) || defined('_EPH_ROOT_DIR_')) {
            ...
            $result = Db::getInstance(_EPH_USE_SQL_SLAVE_)->executeS(
                (new DbQuery())
                    ...
                    ->where('su.admin_ssl = \''.pSQL($host).'\' OR su.domain = \''.pSQL($host).'\' OR su.domain_ssl = \''.pSQL($host).'\'')
                    ->where('s.`active` = 1')
                    ->where('s.`deleted` = 0')
        ...
        if ((!$idShop && defined('_EPH_ROOT_DIR_')) || Tools::isPHPCLI() || in_array($httpHost, $allMedia)) {
            ...
            if (Tools::isPHPCLI() && defined('_EPH_ROOT_DIR_')) {
                ...
                if (!isset($_SERVER['SERVER_NAME']) || empty($_SERVER['SERVER_NAME'])) {
                    $_SERVER['SERVER_NAME'] = $shop->admin_ssl;
                }
	...
	public function getFrontBaseURL($autoSecureMode = false, $addBaseUri = true)
    {
        

        $url = [];
        $url['protocol'] = 'https://';
        $url['admin_ssl'] = $this->domain_ssl;
    ...
    public static function cacheShops($refresh = false)
    {
        ...
        $sql = (new DbQuery())
            ...
            ->select('su.`domain`, su.`domain_ssl`, su.`admin_ssl`, su.`physical_uri`, su.`virtual_uri`')
            ->from('shop_group', 'gs')
